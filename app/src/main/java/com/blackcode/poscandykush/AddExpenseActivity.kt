...
class AddExpenseActivity : AppCompatActivity() {
    ...
    private fun saveExpense() {
        ...
        lifecycleScope.launch(Dispatchers.Main) {
            try {
                val response = createExpense(description, amount, date, time)
                if (response?.success == true) {
                    Toast.makeText(this@AddExpenseActivity, "Expense created successfully", Toast.LENGTH_SHORT).show()
                    setResult(RESULT_OK)
                    finish()
                } else {
                    val errorMessage = response?.error ?: "Unknown error occurred"
                    Toast.makeText(this@AddExpenseActivity, "Failed to create expense: $errorMessage", Toast.LENGTH_LONG).show()
                }
            } catch (e: Exception) {
    ...
    private suspend fun createExpense(
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                apiService.createExpense(token, request)
