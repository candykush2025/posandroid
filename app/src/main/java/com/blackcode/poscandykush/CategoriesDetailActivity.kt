...
class CategoriesDetailActivity : AppCompatActivity() {
    ...
    private fun loadCategoriesData() {
        ...
        if (cachedCategories != null) {
            updateUI(cachedCategories.first)
            showProgress(false)
        ...
        if (data.optBoolean("success", false)) {
            ...
            if (categories != null && categories.length() > 0) {
                for (i in 0 until categories.length()) {
                    ...
                    if (category != null) {
                        val categoryView = createDetailCategoryRow(
                            category.optString("category_name", "Unknown"),
                            category.optInt("quantity_sold", 0),
                            category.optDouble("gross_sales", 0.0),
                            category.optDouble("gross_profit", 0.0),
                            category.optDouble("percentage_of_sales", 0.0)
                        )
                        llCategoriesList.addView(categoryView)
                    }
                }
            } else {
                showEmptyState("No category sales in this period")
            }
        } else {
            showEmptyState("No category data available")
        }
    }
    ...
    private fun showEmptyState(message: String) {
        ...
        val textView = TextView(this).apply {
            text = message
            setTextColor(ContextCompat.getColor(this@CategoriesDetailActivity, R.color.text_secondary))
            textSize = 14f
            setPadding(32, 48, 32, 48)
            gravity = android.view.Gravity.CENTER
        }
        llCategoriesList.addView(textView)
    }

    private fun createDetailCategoryRow(
        name: String,
        quantity: Int,
        sales: Double,
        profit: Double,
        ...
        val categoryView = layoutInflater.inflate(R.layout.item_category_detail_row, null)

        categoryView.findViewById<TextView>(R.id.tv_category_name).text = name
        categoryView.findViewById<TextView>(R.id.tv_category_quantity).text = "Qty: ${NumberFormatter.formatInteger(quantity)}"
        categoryView.findViewById<TextView>(R.id.tv_category_sales).text = NumberFormatter.formatCurrency(sales)
        categoryView.findViewById<TextView>(R.id.tv_category_profit).text = "Profit: ${NumberFormatter.formatCurrency(profit)}"
        categoryView.findViewById<TextView>(R.id.tv_category_percentage).text = "${NumberFormatter.formatPercent(percentage)} of sales"
